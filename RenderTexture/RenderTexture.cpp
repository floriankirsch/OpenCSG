                               ...
                               UpdateMode   updateMode   /* = RT_RENDER_TO_TEXTURE */)
{
  ...
  // [Florian] Even without _bFloat, specify color bits, 
  // else I do not get colors at all (GeForce4MX)
                           ...
                           bool bIsDepthTexture /* = false */)
{
  ...
  // [Florian] reset _bRectangle, else change from POT to NPOT results in error
  _bRectangle = !(IsPowerOfTwo(iWidth) && IsPowerOfTwo(iHeight));
